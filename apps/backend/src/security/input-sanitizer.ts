...
export class InputSanitizer {
  ...
  static sanitizeString(
    ...
    // Remove or escape HTML after security checks
  ...
  static sanitizeUsername(username: string): string {
    ...
    if (sanitized.length > 50) {
      throw new Error('Input exceeds maximum length of 50');
    }

    // Check for username-specific invalid characters first
    ...
    // Now apply general sanitization (this might still detect XSS for safety)
  ...
  static sanitizeFilePath(filePath: string): string {
    ...
    if (sanitized.length > 255) {
      throw new Error('Input exceeds maximum length of 255');
    }
    ...
    // Validate file path format first
    if (!/^[a-zA-Z0-9._/\-]+$/.test(sanitized)) {
      throw new Error('Invalid file path format');
    }

    // Now apply general sanitization for safety
    return this.sanitizeString(sanitized, { maxLength: 255 });
  }
...
export const ValidationSchemas = {
  username: z
    ...
    .min(3)
    .max(50)
    .transform(str => InputSanitizer.sanitizeUsername(str)),
  email: z
    .string()
    .min(1)
    .max(320)
    .transform(str => InputSanitizer.sanitizeEmail(str)),
  aiPrompt: z
    .string()
    .min(1)
    .max(2000)
    .transform(str => InputSanitizer.sanitizeAIPrompt(str)),
  filePath: z
    .string()
    .min(1)
    .max(255)
    .transform(str => InputSanitizer.sanitizeFilePath(str)),
  safeString: z
    .string()
    .max(1000)
    .transform(str => InputSanitizer.sanitizeString(str)),
};
