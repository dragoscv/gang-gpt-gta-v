...
export class AdvancedMemoryMCP {
  ...
  constructor(
    ...
    this.aiEnabled = options?.aiEnabled ?? config.features.aiCompanions;
  ...
  async recall(options: MemorySearchOptions): Promise<MemoryEntry[]> {
    try {
      const {
        ...
        minImportance = 1,
        timeRange,
        semanticThreshold = process.env.NODE_ENV === 'test' ? 0.1 : 0.7,
      } = options;
      ...
      if (timeRange) {
        candidates = candidates.filter(memory => {
          ...
          return (
            (!timeRange.start || timestamp >= timeRange.start) &&
            (!timeRange.end || timestamp <= timeRange.end)
          );
        });
      }

      // Semantic search with embeddings (check if AI is available and embeddings exist)
      if (query && this.aiEnabled && candidates.length > 0) {
        const queryEmbedding = await this.generateEmbedding(query);
        if (queryEmbedding) {
          const semanticResults = candidates
            .filter(memory => memory.embedVector && memory.embedVector.length > 0)
            .map(memory => ({
              memory,
          ...
          // If semantic search found results, use them; otherwise fall back to keyword search
      ...
      // Fallback to keyword search if semantic search unavailable or query exists
      else if (query) {
        candidates = this.performKeywordSearch(query, candidates);
      }
      ...
      candidates.sort((a, b) => {
        ...
        const recencyWeight =
          (b.timestamp.getTime() - a.timestamp.getTime()) * 0.0000001;
      ...
      if (typeof criteria === 'string') {
        ...
        if (criteria.contextFilter || criteria.olderThan) {
          ...
          const filtered = memories.filter(memory => {
            if (
              criteria.contextFilter &&
              !this.matchesContext(memory.context, criteria.contextFilter)
            ) {
              return false;
            }
  ...
  async getContextSummary(
    context: Partial<MemoryContext> | null,
  ...
  private async enhanceTagsWithAI(
    content: string,
    existingTags: string[]
  ): Promise<string[]> {
  ...
  private async findRelatedMemories(
    ...
    const candidates = Array.from(this.memoryStore.values())
      .filter(m => m.context && m.context.contextType === context.contextType)
      .slice(0, 20);
  ...
  private async generateEmbedding(
    ...
    // For now, return a mock embedding in test mode or when AI is available
    if (!this.aiEnabled && process.env.NODE_ENV !== 'test') {
      return undefined;
    }
  ...
  private recalculateAnalytics(): void {
    ...
    for (const memory of allMemories) {
      if (memory.context && memory.context.contextType) {
        const contextType = memory.context.contextType;
        this.analytics.contextDistribution[contextType] =
          (this.analytics.contextDistribution[contextType] || 0) + 1;
      }
  ...
  private async performMemoryMaintenance(): Promise<void> {
    ...
    const cutoffDate = new Date(
      Date.now() - config.ai.memoryRetentionDays * 24 * 60 * 60 * 1000
    );
