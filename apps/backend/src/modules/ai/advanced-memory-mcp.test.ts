...
describe('AdvancedMemoryMCP', () => {
  ...
  describe('Memory Retrieval', () => {
    ...
    it('should recall memories with keyword search', async () => {
      // Use a non-AI instance to ensure keyword search is used
      const keywordMCP = new AdvancedMemoryMCP(undefined, { aiEnabled: false });
      
      // Populate with the same test data
      await keywordMCP.remember('Player Alice joined faction Alpha', testContext, 8, ['player', 'faction', 'join']);
      await keywordMCP.remember('Mission completed successfully', testContext, 9, ['mission', 'success']);
      await keywordMCP.remember('Weather changed to rainy', testContext, 3, ['weather', 'environment']);
      await keywordMCP.remember('Player Bob defeated enemy', testContext, 7, ['player', 'combat', 'victory']);
      await keywordMCP.remember('New AI companion acquired', testContext, 6, ['ai', 'companion', 'acquisition']);

      const results = await keywordMCP.recall({
        query: 'player',
        limit: 10,
      });
  ...
  describe('Semantic Search', () => {
    ...
    it('should fall back to keyword search when semantic search is unavailable', async () => {
      ...
      const noAiMemoryMCP = new AdvancedMemoryMCP(undefined, { aiEnabled: false });
