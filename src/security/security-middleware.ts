...
export class SecurityMiddleware {
   ...
   */
  static initialize(options: SecurityOptions = {}) {
    const {
        ...
        store: {
          increment: (key: string) => {
            ...
            if (!record || now > record.resetTime) {
              ...
              return Promise.resolve({ totalHits: 1, resetTime: new Date(now + config.security.rateLimitWindowMs) });
            }
   ...
   * Input sanitization middleware
   */
  static sanitizeInputs = (req: Request, res: Response, next: NextFunction): void => {
    try {
   ...
   */
  static auditLogger = (req: Request, res: Response, next: NextFunction): void => {
  ...
  static aiRateLimit = rateLimit({
    ...
    message: {
      error: 'AI request rate limit exceeded. Please wait before making more AI requests.',
    },
   ...
   */
  static httpsRedirect = (req: Request, res: Response, next: NextFunction): void => {
    if (config.server.environment === 'production' && !req.secure && req.get('x-forwarded-proto') !== 'https') {
   ...
   */
  static apiKeyValidation = (req: Request, res: Response, next: NextFunction) => {
